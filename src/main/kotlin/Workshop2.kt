...
//    println("   - การคำนวณจะเกิดขึ้นเมื่อมี 'Terminal Operation' มาเรียกใช้เท่านั้น (ในที่นี้คือ .sum())")
    ...
    //WorkShop 2.2
    val totalPriceOver10000 = products.asSequence()
        .filter {it.price > 10000 }

    val less1000 = products.asSequence()
        .filter {it.price < 1000 }

    val between10009999 = products.asSequence()
        .filter {it.price > 1000 && it.price < 9999}

    println("กลุ่มสินค้าตามราคา:")
    println("10,000 ขึ้นไป:")
    totalPriceOver10000.forEach{ println( "- ${it.name}: ${it.price} " ) }
    println("--------------------------------------------------")

    println("ไม่เกิน 1,000 บาท:")
    less1000.forEach{ println( "- ${it.name}: ${it.price} " ) }
    println("--------------------------------------------------")
